...
class ExtensionBuilder(ExtensionBuilderBase):
	...
	def _patch_file(self, path):
		...
		with codecs.open(path, 'r+', encoding) as f:
			...
			if header is None and not self.is_content_script(self._info, path) and not self.is_background_script(self._info, path):
				...
				content = re.sub("'kango-old", "'" + old_name + '_kango', content, flags=re.IGNORECASE)
