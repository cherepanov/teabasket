...
http://kangoextensions.com/
*/
ChristmasTree_kango.UserscriptEngine=function(){this._scripts=[];};ChristmasTree_kango.UserscriptEngine.prototype={_scripts:[],addScript:function(id,text){for(var i=0;i<this._scripts.length;i++){if(this._scripts[i].id==id){return false;}}
this._scripts.push({id:id,script:new ChristmasTree_kango.Userscript(text)});return true;},removeScript:function(id){for(var i=0;i<this._scripts.length;i++){if(this._scripts[i].id==id){this._scripts.splice(i,1);return true;}}
...
return this._checkPatternArray(script.headers.exclude,url);}};ChristmasTree_kango.Userscript=function(text){this.text=text;this.headers={};this._parseHeaders();};ChristmasTree_kango.Userscript.prototype={headers:null,text:null,_parseHeaders:function(){this.headers=this._parseHeadersToHashTable(this.text);if(typeof this.headers.match!='undefined'){if(typeof this.headers.include=='undefined'){this.headers.include=this.headers.match;}
...
return headers;}};ChristmasTree_kango.registerModule(function(){this.init=function(){ChristmasTree_kango.userscript=new ChristmasTree_kango.UserscriptEngine();var content_scripts=ChristmasTree_kango.getExtensionInfo().content_scripts;if(typeof content_scripts!='undefined'){for(var i=0;i<content_scripts.length;i++){ChristmasTree_kango.userscript.addScript(content_scripts[i],ChristmasTree_kango.io.getExtensionFileContents(content_scripts[i]));}}};});

// Merged from D:\Work\Kango-Builds\kango-0.9.8-public\src\js\ie firefox\ChristmasTree_kango\userscript_engine.part.js

/*
Built using ChristmasTree_kango - Cross-browser extension framework.
http://kangoextensions.com/
*/
ChristmasTree_kango.addEventListener(ChristmasTree_kango.event.Ready,function(){ChristmasTree_kango.browser.addEventListener('DOMContentLoaded',function(event){var gm_client=new ChristmasTree_kango.UserscriptEngineClient();gm_client.run(event.window);});});