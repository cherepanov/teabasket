...
http://kangoextensions.com/
*/
var GmailnChecker_kango=null;var GmailnChecker_kangoPopup={_container:null,border_size:21,frameWidth:0,frameHeight:0,x:0,y:0,url:'',setConfig:function(config){this.url=config.url;this.frameWidth=config.width;this.frameHeight=config.height;this.x=config.x;this.y=config.y;},getFrame:function(){return document.getElementById('frame');},navigateFrame:function(url){this.getFrame().setAttribute('src',url);},initApiForFrame:function(){var self=this;this.getFrame().addEventListener('DOMContentLoaded',function(){(new XPCNativeWrapper(self.getFrame().contentWindow).wrappedJSObject)['kango']=GmailnChecker_kango;},true);},init:function(){var config=window.arguments[0].wrappedJSObject;GmailnChecker_kango=config['kango'];this.setConfig(config);this.initApiForFrame();this.navigateFrame(this.url);var outerSize=this.getOuterSize();window.resizeTo(outerSize.width,outerSize.height);this.magicMoveTo(this.x,this.y);},getOuterSize:function(){return{width:this.frameWidth+2*this.border_size,height:this.frameHeight+2*this.border_size};},getWindowRect:function(wnd){var rect=null;if(wnd.windowState==STATE_MAXIMIZED){rect={left:wnd.mozInnerScreenX,top:wnd.mozInnerScreenY,right:wnd.screenX+(wnd.mozInnerScreenX-wnd.screenX)+wnd.innerWidth,bottom:wnd.screenY+(wnd.mozInnerScreenY-wnd.screenY)+wnd.innerHeight};}
...
additionalPinIndent=5+6;var pin=document.getElementById('pin');pin.left=(x-standartPos.x-halfPinWidth-additionalPinIndent)+'px';window.moveTo(standartPos.x,standartPos.y);}};window.addEventListener('load',function(){GmailnChecker_kangoPopup.init()},false);