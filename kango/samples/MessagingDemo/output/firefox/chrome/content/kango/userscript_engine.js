...
*/
MessagingDemo_kango.UserscriptEngine=function(){this._scripts=[];};MessagingDemo_kango.UserscriptEngine.prototype={_scripts:[],addScript:function(id,text){for(var i=0;i<this._scripts.length;i++){if(this._scripts[i].id==id){return false;}}
this._scripts.push({id:id,script:new MessagingDemo_kango.Userscript(text)});return true;},removeScript:function(id){for(var i=0;i<this._scripts.length;i++){if(this._scripts[i].id==id){this._scripts.splice(i,1);return true;}}
...
return this._checkPatternArray(script.headers.exclude,url);}};MessagingDemo_kango.Userscript=function(text){this.text=text;this.headers={};this._parseHeaders();};MessagingDemo_kango.Userscript.prototype={headers:null,text:null,_parseHeaders:function(){this.headers=this._parseHeadersToHashTable(this.text);if(typeof this.headers.match!='undefined'){if(typeof this.headers.include=='undefined'){this.headers.include=this.headers.match;}
...
return headers;}};MessagingDemo_kango.registerModule(function(){this.init=function(){MessagingDemo_kango.userscript=new MessagingDemo_kango.UserscriptEngine();var content_scripts=MessagingDemo_kango.getExtensionInfo().content_scripts;if(typeof content_scripts!='undefined'){for(var i=0;i<content_scripts.length;i++){MessagingDemo_kango.userscript.addScript(content_scripts[i],MessagingDemo_kango.io.getExtensionFileContents(content_scripts[i]));}}};});

// Merged from D:\Work\Kango-Builds\kango-0.9.8-public\src\js\ie firefox\MessagingDemo_kango\userscript_engine.part.js

/*
Built using MessagingDemo_kango - Cross-browser extension framework.
http://kangoextensions.com/
*/
MessagingDemo_kango.addEventListener(MessagingDemo_kango.event.Ready,function(){MessagingDemo_kango.browser.addEventListener('DOMContentLoaded',function(event){var gm_client=new MessagingDemo_kango.UserscriptEngineClient();gm_client.run(event.window);});});