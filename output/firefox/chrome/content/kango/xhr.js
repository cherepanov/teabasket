...
http://kangoextensions.com/
*/
TeeBasket_kango.XHRRequest=function(){this.method='GET';this.url='';this.params={};this.headers={};this.async=true;this.contentType='';this.username='';this.password='';this.mimeType='';};TeeBasket_kango.XHRResult=function(){this.response='';this.status=0;};TeeBasket_kango.XHR=function(){};TeeBasket_kango.XHR.prototype={_paramsToString:function(params){var result='';for(var key in params){if(params.hasOwnProperty(key)){if(result!=''){result+='&';}
result+=key+'='+params[key];}}
return result;},getXMLHttpRequest:function(){return new XMLHttpRequest();},send:function(details,callback){var req=this.getXMLHttpRequest();var method=details.method||'GET';var async=details.async||true;var params=details.params||null;var type=details.contentType||'text';var url=details.url;var username=details.username||null;var password=details.password||null;var headers=details.headers||{};var mimeType=details.mimeType||null;if(url.indexOf(TeeBasket_kango.SCHEME)==0||(url.indexOf('http://')==-1&&url.indexOf('https://')==-1)){url=TeeBasket_kango.io.getFileUrl(url);}
...
return getResult(req,type);}};TeeBasket_kango.xhr=new TeeBasket_kango.XHR();